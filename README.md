...
### Центр изменений / Dry Run

Перед массовыми операциями, сменой Static/On demand и временным отключением панель сначала строит план без PUT/DELETE: показывает поток, CDN, изменяемое поле и значения «до → после». Применение запускается отдельной кнопкой. Dry Run не создаёт backup и ничего не записывает во Flussonic.

### Карточка канала

Кнопка `▣` в строке потока открывает единую карточку: конфигурация, размещение, состояние на всех CDN, clients, input/output bitrate, inputs, последние изменения Audit и backup. Из карточки доступны Preview, Inputs, Диагностика и Редактирование.
...
Архив загрузите как `/root/flussonic-panel-v5.10.zip`, затем запустите поставляемый скрипт:

```bash
rm -rf /tmp/flussonic-panel-v510-installer
mkdir -p /tmp/flussonic-panel-v510-installer
unzip -q -o /root/flussonic-panel-v5.10.zip -d /tmp/flussonic-panel-v510-installer
bash /tmp/flussonic-panel-v510-installer/flussonic-panel/upgrade-from-zip.sh
```

Скрипт сохраняет `.git` и `.env`, не использует `rsync`, пересобирает отдельный образ `flussonic-panel:v510` и оставляет Docker volume `flussonic-panel-data` без изменений.
...
```json
{"ok":true,"version":"5.10.0","cluster":true,"placement":true}
```

Откройте:

```text
http://IP_ПАНЕЛИ:8088/?v=594
```
